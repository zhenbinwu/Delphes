...
public:
   ...
   virtual bool pass(const fastjet::PseudoJet &) const {
	if (!applies_jet_by_jet())	
	  throw fastjet::Error("Cannot apply this selector worker to an individual jet");
	return false;
   }

   virtual void terminator(vector<const fastjet::PseudoJet *> & jets) const {
	//copy pointers content to a vector of PseudoJets. Check if the pointer has been already nullified. 
	vector<fastjet::PseudoJet> my_jets;
...
fastjet::Selector SelectorEventTrimmer(double Rjet, double ptcut, double Rsub, double fcut){
  return fastjet::Selector(new SW_EventTrimmer(Rjet,ptcut,Rsub,fcut));
} 
*/
...
double JetMultiplicity::result(const vector<fastjet::PseudoJet> & particles) const {
...
double TransverseEnergy::result(const vector<fastjet::PseudoJet> & particles) const {
...
double MissingTransverseEnergy::result(const vector<fastjet::PseudoJet> & particles) const {

   fastjet::PseudoJet p_tot(0,0,0,0);
...
double pt_within_R(const vector<fastjet::PseudoJet> & particles, const fastjet::PseudoJet& centre, double R){
...
//    }
    ...
    for(unsigned int i=0; i<near_particles.size(); i++){
        ...
        fastjet::PseudoJet lSum = centre + near_particles[i];
